...
class H2Currency(
    ...
    override suspend fun getSymbol(): String {
        return handler.connection.prepareStatement(H2Statements.getSymbolForCurrency).use { statement ->
    ...
    override suspend fun getDecimal(locale: Locale): String {
        return handler.connection.prepareStatement(H2Statements.getDecimalForCurrencyWithLocale).use { statement ->
    ...
    override suspend fun getLocaleDecimalMap(): Map<Locale, String> {
        return handler.connection.prepareStatement(H2Statements.getLocaleDecimalPairsForCurrency).use { statement ->
            val rs = statement.executeQuery()
    ...
    override suspend fun getDisplayName(plural: Boolean, locale: Locale): String {
        return handler.connection.prepareStatement(H2Statements.getDisplayNamesForCurrencyWithLocale).use { statement ->
            statement.setString(1, locale.toLanguageTag())
            val rs = statement.executeQuery()
            return@use if (rs.next()) {
                rs.getString(
    ...
    override suspend fun isPrimary(): Boolean {
        return handler.getPrimaryCurrency().name == name
    }

    override suspend fun getStartingBalance(): BigDecimal {
        return handler.connection.prepareStatement(H2Statements.getStartingBalanceForCurrency).use { statement ->
            val rs = statement.executeQuery()
            return@use if (rs.next()) {
                rs.getBigDecimal(1)
            } else {
    ...
    override suspend fun getConversionRate(): BigDecimal {
        return handler.connection.prepareStatement(H2Statements.getConversionRateForCurrency).use { statement ->
            val rs = statement.executeQuery()
            return@use if (rs.next()) {
                rs.getBigDecimal(1)
            } else {
