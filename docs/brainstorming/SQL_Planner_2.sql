...
CREATE TABLE IF NOT EXISTS arceco_accounts(
    ...
    type ENUM('Player', 'NonPlayer') NOT NULL
)
...
CREATE TABLE IF NOT EXISTS arceco_player_accounts (
    ...
    FOREIGN KEY(account_id) REFERENCES arceco_accounts(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS arcoeco_account_balances (
    ...
    FOREIGN KEY(account_id) REFERENCES arceco_accounts(id) ON DELETE CASCADE,
    FOREIGN KEY(currency_id) REFERENCES arceco_currencies(id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS arceco_account_members (
    account_id INT NOT NULL,
    member_uuid VARCHAR(36) NOT NULL,
    permission ENUM('BALANCE', 'WITHDRAW', 'DEPOSIT', 'MODIFY_PERMISSIONS') NOT NULL,
...
CREATE TABLE IF NOT EXISTS arceco_transactions (
    id INT NOT NULL PRIMARY KEY,
    account_id INT NOT NULL,
    initiator_type ENUM('PLAYER', 'SERVER', 'PLUGIN') NOT NULL,
    initiator_data VARCHAR(36) NOT NULL,
