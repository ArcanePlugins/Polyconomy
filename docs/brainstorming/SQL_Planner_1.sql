...
CREATE TABLE IF NOT EXISTS ArcEco_PlayerCache (
    ...
    player_username VARCHAR(48) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS ArcEco_Currencies (
    ...
    currency_name VARCHAR(48) NOT NULL UNIQUE
)
...
CREATE TABLE IF NOT EXISTS ArcEco_PlayerAccounts (
    ...
    player_uuid VARCHAR(32) NOT NULL UNIQUE
);
...
CREATE TABLE IF NOT EXISTS ArcEco_PlayerAccountBalances (
    ...
    FOREIGN KEY(account_id) REFERENCES ArcEco_PlayerAccounts(account_id) ON DELETE CASCADE,
    FOREIGN KEY(currency_id) REFERENCES ArcEco_Currencies(currency_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS ArcEco_NonPlayerAccounts (
    ...
    account_id INT PRIMARY KEY,
    account_name VARCHAR(48) NOT NULL UNIQUE
);
...
CREATE TABLE IF NOT EXISTS ArcEco_NonPlayerAccountBalances (
    ...
    currency_id INT NOT NULL,
    balance DECIMAL(38,5) NOT NULL,
    FOREIGN KEY(account_id) REFERENCES ArcEco_NonPlayerAccounts(account_id) ON DELETE CASCADE,
    FOREIGN KEY(currency_id) REFERENCES ArcEco_Currencies(currency_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS ArcEco_NonPlayerAccountMembers (
    ...
    account_id INT NOT NULL,
    member_uuid VARCHAR(32) NOT NULL,
...
REPLACE INTO PlayerCache VALUES('abc-def-ghi-jkl', 'Notch', 'no clue');
INSERT INTO PlayerCache VALUES('abc-def-ghi-jkl', 'lokka30', 'no clue');

/* Update row(s) */
UPDATE PlayerCache SET player_username = 'Notch' WHERE player_uuid = 'abc-def-ghi-jkl';

/* Select specific columns */
SELECT player_uuid, player_username, cache_timestamp FROM PlayerCache;

/* Delete account */
DELETE FROM NonPlayerAccounts WHERE account_name = 'TownyAccount';

/* Baltop */
SELECT * FROM PlayerAccountBalances WHERE currency_id = 'dollars' ORDER BY balance;

/* Server balance */
SELECT SUM(balance) FROM ArcEco_PlayerAccountBalances WHERE currency_id = 'dollars';
